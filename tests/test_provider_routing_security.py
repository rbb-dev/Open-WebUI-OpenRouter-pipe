...
class TestSafeLiteralString:
    ...
    def test_empty_string(self):
        ...
        result = FilterManager.safe_literal_string("")
        assert result in ("''", '""')

    def test_none_input(self):
        """None input should return empty quoted string."""
        result = FilterManager.safe_literal_string(None)
        assert result in ("''", '""')

    def test_non_string_input(self):
        """Non-string input should be converted to string."""
        result = FilterManager.safe_literal_string(123)
        assert result in ("'123'", '"123"')

    def test_unicode_characters(self):
        """Unicode characters should be handled safely."""
        result = FilterManager.safe_literal_string("café ☕")
        parsed = ast.literal_eval(result)
        assert parsed == "café ☕"

    def test_backslash_escaping(self):
        """Backslashes should be properly escaped."""
        result = FilterManager.safe_literal_string("path\\to\\file")
        parsed = ast.literal_eval(result)
        assert parsed == "path\\to\\file"


class TestValidateProviderName:
    """Tests for FilterManager.validate_provider_name()."""

    def test_simple_name(self):
        """Simple provider names should pass through."""
        assert FilterManager.validate_provider_name("Amazon Bedrock") == "Amazon Bedrock"

    def test_removes_special_characters(self):
        """Special characters should be removed."""
        result = FilterManager.validate_provider_name("Evil<script>Provider")
    ...
    def test_removes_quotes(self):
        ...
        result = FilterManager.validate_provider_name('Test"Provider\'Name')
        assert '"' not in result
        assert "'" not in result

    def test_collapses_whitespace(self):
        """Multiple spaces/underscores should be collapsed."""
        result = FilterManager.validate_provider_name("Too    Many   Spaces")
        assert "    " not in result

    def test_empty_input(self):
        """Empty input should return 'Unknown'."""
        assert FilterManager.validate_provider_name("") == "Unknown"
        assert FilterManager.validate_provider_name(None) == "Unknown"
        assert FilterManager.validate_provider_name("   ") == "Unknown"
    ...
    def test_truncation_preserves_uniqueness(self):
        ...
        name1 = "A" * 100
        name2 = "B" * 100
        result1 = FilterManager.validate_provider_name(name1, slug="slug1")
        result2 = FilterManager.validate_provider_name(name2, slug="slug2")
...
class TestValidateFilterSource:
    """Tests for FilterManager.validate_filter_source()."""
    ...
    def test_empty_source(self):
        ...
        is_valid, error = FilterManager.validate_filter_source("")
        assert is_valid is False
        assert error is not None and ("Empty" in error or "invalid" in error.lower())

    def test_none_source(self):
        """None source should fail."""
        is_valid, error = FilterManager.validate_filter_source(None)
        assert is_valid is False
...
class TestRenderProviderRoutingFilterSourceSecurity:
    ...
    def test_empty_model_slug_raises(self):
        ...
        with pytest.raises(ValueError, match="non-empty string"):
            FilterManager._render_provider_routing_filter_source(
                model_slug="",
                providers=["openai"],
                quantizations=[],
                visibility="user",
...
class TestGeneratedInletLogic:
    ...
    def test_admin_visibility_logic(self):
        ...
        logic = FilterManager._generate_inlet_logic("admin")
    ...
    def test_both_visibility_logic(self):
        ...
        logic = FilterManager._generate_inlet_logic("both")
        assert "self.valves" in logic
        assert "user_valves" in logic
        assert '__user__.get("valves")' in logic or "__user__.get('valves')" in logic
