...
@pytest.mark.asyncio
async def test_is_safe_url_allows_public_ips(pipe_instance_async, monkeypatch):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("https://example.com/image.jpg") is True
...
async def test_is_safe_url_blocks_private_ranges(pipe_instance_async, monkeypatch, ip):
...
async def test_is_safe_url_blocks_literal_loopback(pipe_instance_async):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("https://127.0.0.1/internal") is False
...
async def test_http_blocked_by_default(pipe_instance_async, monkeypatch):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("http://example.com/resource") is False
...
async def test_http_allowlist_allows_host(pipe_instance_async, monkeypatch):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("http://example.com/resource") is True
...
async def test_http_allowlist_port_mismatch_blocks(pipe_instance_async, monkeypatch):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("http://example.com:80/resource") is False


@pytest.mark.asyncio
async def test_is_safe_url_handles_dns_failures(pipe_instance_async, monkeypatch):
    """DNS errors should fall back to 'unsafe'."""
    def _raise(*_args, **_kwargs):
        raise socket.gaierror

    monkeypatch.setattr(socket, "getaddrinfo", _raise)
    assert await pipe_instance_async._multimodal_handler._is_safe_url("https://unknown.invalid") is False


@pytest.mark.asyncio
async def test_is_safe_url_rejects_urls_without_hostname(pipe_instance_async):
    """file:// and data: URLs should be rejected up front."""
    assert await pipe_instance_async._multimodal_handler._is_safe_url("file:///etc/passwd") is False
    assert await pipe_instance_async._multimodal_handler._is_safe_url("data:text/plain,hello") is False
...
async def test_is_safe_url_respects_disabled_valve(pipe_instance_async):
    ...
    assert await pipe_instance_async._multimodal_handler._is_safe_url("https://127.0.0.1/admin") is True
...
def test_is_youtube_url_variants(pipe_instance):
    ...
    assert pipe_instance._multimodal_handler._is_youtube_url("https://www.youtube.com/watch?v=dQw4w9WgXcQ") is True
    assert pipe_instance._multimodal_handler._is_youtube_url("https://youtu.be/dQw4w9WgXcQ") is True
