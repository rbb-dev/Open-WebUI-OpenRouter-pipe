...
| `ENABLE_SSRF_PROTECTION` | `bool` | `True` | Enable SSRF protection for remote URL downloads. When enabled, an address is fetched only if it is provably globally routable, so loopback, RFC1918, link-local, carrier-grade NAT (`100.64.0.0/10`, Tailscale's default range), IPv6 site-local and anything else not marked globally routable are all refused; IPv6 forms carrying an IPv4 address (`::ffff:`, 6to4, Teredo, NAT64) are judged on the addresses inside, and a form carrying more than one (Teredo carries the tunnel server as well as the client) is allowed only when every one of them would be allowed on its own. HTTPS-only defaults still apply even if SSRF protection is disabled. |
...
| `ENABLE_SUBAGENT` | `bool` | `True` | Enable the OpenRouter Subagent server tool (delegate tasks to a worker model an admin chooses; extra model call, default-off per chat). When disabled, subagent toggles are hidden from users. |
...
| `VIDEO_MAX_POLL_TIME_SECONDS` | `int` | `1800` | How long a job may go without a word from OpenRouter before polling stops. The clock restarts on every status check that reports the job still running, so a long render is never cut off for taking a long time. On expiry the job card is persisted as still running rather than failed, and the effective wait is floored at `VIDEO_POLL_INTERVAL_MAX_SECONDS` plus the HTTP read timeout. |
