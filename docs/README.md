...
- [OpenRouter Image Generation](openrouter_image_generation.md) — native image-output models (Sourceful, FLUX, Seedream, Gemini Image, GPT-5 Image), one settings panel per model, built from what that model publishes, per-model knobs, and the inline rendering pipeline.
