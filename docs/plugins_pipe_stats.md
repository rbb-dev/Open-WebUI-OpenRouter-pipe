...
├── ephemeral_keys.py     # Ephemeral key store for SSE authentication
...
The plugin ships with two built-in commands:
...
SSE connections are authenticated using process-local ephemeral keys generated by `EphemeralKeyStore`:

1. **Key generation**: When the `stats` command fires, `EphemeralKeyStore.generate()` creates a 256-bit cryptographic key via `secrets.token_hex(32)` and stores it with a timestamp.
2. **Key embedding**: The key is embedded in the HTML dashboard's JavaScript as a URL parameter: `EventSource('/api/pipe/stats/{KEY}')`.
3. **Key validation**: The SSE endpoint validates the key on connection. Invalid or expired keys return HTTP 403.
4. **TTL expiry**: Keys expire after 5 minutes of inactivity. The store caps at 10 active keys to prevent resource exhaustion.
5. **Process-local**: Keys are stored in a plain `dict` (not Redis) — they only work on the worker process that generated them. This is intentional: the dashboard HTML and the SSE endpoint are always served by the same process.
