...
CONFIG_META: dict[str, dict[str, str]] = {
    ...
    "ENABLE_OPENROUTER_FUSION": {
        ...
        "group": "Filters & Integrations/Fusion",
        "detail": "Adds an `openrouter/fusion` model to the picker that answers a hard prompt by deliberation, not in a single pass - a panel of up to eight models replies in parallel, a judge model weighs them for consensus, contradictions, and gaps, and a final model writes the answer from that analysis.\n\nThis is Fusion's master switch: while on, that model stays in the picker and every chat on it deliberates - the pipe adds the activating `fusion` plugin entry and requires the deliberation tool on each request, so picking the fusion model always means a real panel run (chat-title and other housekeeping requests never have Fusion added or required, so they can't bill a panel). Each chat shows the panel's takes streaming live, any model's collapsible thinking, the judge's analysis, and the final answer. The preset, panel models, and judge are per-chat controls on the OpenRouter Fusion filter. Running a whole panel plus a judge for one answer costs far more than a normal completion - reserve it for research or high-stakes questions, and check OpenRouter's pricing. Turn it off and Fusion is off for everyone - no filter, no activation, no deliberation."
    },
    ...
    "REMOTE_VIDEO_MAX_SIZE_MB": {
        ...
        "group": "Files & Media/Video Generation",
        "detail": "The largest a generated video may grow, in megabytes, before its download is aborted and the generation job fails.\n\nThe default fits typical short generated clips. This same limit also caps each clip or sound file attached to a request as a reference, where one that exceeds it is left out with a note in the chat rather than failing the render. Raise it for longer or higher-resolution outputs; set it below the real file size and a finished video is discarded before the download completes, surfacing as a failed job rather than a delivered result."
    },
    ...
    "VIDEO_FRAME_IMAGE_MAX_BYTES": {
        ...
        "group": "Files & Media/Uploads & Limits",
        "detail": "How large a single image frame may be, in bytes, when the pipe encodes it and sends it into a video generation request.\n\nThe default covers typical source stills. Any frame whose decoded size passes this cap fails the whole video request, so too low a value turns away legitimate images. Raise it to allow larger source frames, or lower it to reject oversized uploads sooner. It also caps each picture sent only as a reference, where an oversized one is left out with a note in the chat instead of failing the render.\n\n**Tip:** This limit applies to each frame on its own; a request with several frames can still be turned down by `Maximum combined frame size`, which limits the combined size of all frames."
    },
