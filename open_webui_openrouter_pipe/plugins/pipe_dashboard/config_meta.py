...
CONFIG_META: dict[str, dict[str, str]] = {
    ...
    "ENABLE_OPENROUTER_FUSION": {
        ...
        "group": "Filters & Integrations/Fusion",
        "detail": "Adds an `openrouter/fusion` model to the picker that answers a hard prompt by deliberation, not in a single pass — a panel of up to eight models replies in parallel, a judge model weighs them for consensus, contradictions, and gaps, and a final model writes the answer from that analysis.\n\nThis is Fusion's master switch: while on, that model stays in the picker and every chat on it deliberates — the pipe adds the activating `fusion` plugin entry and requires the deliberation tool on each request, so picking the fusion model always means a real panel run (chat-title and other housekeeping requests are never injected or forced, so they can't bill a panel). Each chat shows the panel's takes streaming live, any model's collapsible thinking, the judge's analysis, and the final answer. The preset, panel models, and judge are per-chat controls on the OpenRouter Fusion filter. Running a whole panel plus a judge for one answer costs far more than a normal completion — reserve it for research or high-stakes questions, and check OpenRouter's pricing. Turn it off and Fusion is off for everyone — no filter, no activation, no deliberation."
    },
